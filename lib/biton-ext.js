  ...
  class bitonExtension extends EventEmitter {
    ...
    onExtendedHandshake (handshake) {
      ...
      bitonCrypto.ready(function () {
        this._ownSolution = bitonCrypto.blake2b_256(Uint8Array.from([this._challengeSeed, this._ownPeerId, this._otherPeerId].join(' ')))
        this._remoteChallenge =  bitonCrypto.blake2b_256(Uint8Array.from([this._challengeSeed, this._otherPeerId, this._ownPeerId].join(' ')))

        // Add services bitfield (8 bytes, uint64_t)
