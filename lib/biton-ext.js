  ...
  class bitonExtension extends EventEmitter {
    ...
    onExtendedHandshake (handshake) {
      ...
      if (this.initiator) {
        ...
        try {
          ...
          debug('could not create Noise XX message A, abort: %s', err)
          this.abort = true
          return
        }
    ...
    onMessage (buf) {
      ...
      if (this._noise.isReady) {
        debug('received noise message from peer %s %s', this._remotePeerId)
      } else {
    ...
    _handleNoiseHandshakeMsg(dictRecv)
    {
      if (!this._passedChallenge) {
        ...
        if (!dictRecv.challenge || dictRecv.challenge !== this._localChallenge) {
          debug('received invalid challenge message during handshake, abort')
          this.abort = true
          return
        }
        ...
        if (!this.initiator) {
          ...
          try {
            ...
            debug('could not create Noise_XX message A, abort: %s', err)
            this.abort = true
            return
          }
          ...
          try {
            ...
            debug('could not parse Noise_XX message B, abort: %s', err)
            this.abort = true
            return
          }
        ...
        try {
          ...
          debug('could not parse Noise_XX message C, abort: %s', err)
          this.abort = true
          return
        }
        this._announceNoise()
    ...
    _announceNoise () {
      debug('noise ready for wire with peer %s', this._remotePeerId)
